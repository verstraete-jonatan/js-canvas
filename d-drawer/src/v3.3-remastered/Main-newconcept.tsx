...
class SphereX {
  ...
  mutate(
    ...
    this.pts.forEach(([x, y, z], id) => {
      ...
      const relId = id / totalNrPoints + ogInc;
      // y /=
...
const Main = () => {
  ...
  const updateMesh = useCallback(() => {
    if (meshRef.current) {
      sphere.mutate(meshRef.current, controlA, controlB, controlC, og / ogInc);

      meshRef.current.instanceMatrix.needsUpdate = true;
    }
  }, [meshRef.current, controlA, controlB, controlC, ogInc]);
