const main = (nrPoints = 0) => {
  const totalToDraw = 100 * 1000;

  const createTriangle = (x = 1, y = 1, s = 200) => {
    const res = [];
    const a = (Math.PI * 2) / nrPoints;

    for (let i = 0; i < nrPoints; i++) {
      const rotate = degRad(-90 + i);
      res.push({
        x: x + s * Math.cos(a * i + rotate),
        y: y + s * Math.sin(a * i + rotate),
      });
    }

    return res;
  };

  const points = createTriangle(Xmax + Xmid, Ymax + Ymid, 500);
  let current = points[0];

  const setNew = ({ x, y }) => {
    current = {
      x: (x - current.x) / PI,
      y: (y - current.y) / PI,
    };
  };

  const draw = () => {
    const r = randint(nrPoints - 1);
    setNew(points[r]);
    point(current.x, current.y, 2, COLORS[r]);
    // point(current.x, current.y, 2, hsl(mapNum(r, 0, nrPoints, 0, 360)));
  };

  for (let i = 0; i < totalToDraw; i++) {
    draw();
  }
};

let i = 0;
const animate = async () => {
  if (i <= 900) {
    clear();
    main(++i);
    requestAnimationFrame(animate);
  }
};

animate();
