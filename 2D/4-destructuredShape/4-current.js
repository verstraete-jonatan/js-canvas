...
async function getShape(image) {
  ...
  const fullRes = arrayChuck(res.sortAc("x", "y"), 2); //.map(i=>makeEquilateralTriangle(i[0], i[1]), shapemid)
...
const Main = async () => {
  const imgs = [
    { src: "brain.png", loss: 148, size: 0.6 },
    { src: "face.png", loss: 8, size: 1 },
    { src: "hand.png", loss: 200, size: 0.5 },
  ];
  let idx = 0;
  while (!exit) {
