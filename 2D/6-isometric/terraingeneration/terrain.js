...
class Grid {
  ...
  async dev2() {
    let incx = 0;
    let incxStop = [12, 36, 60];
    for (let py of this.grid) {
      for (let px of py) {
        let { x, y, v } = px;

        if (incxStop.includes(incx)) {
          ctx.lineWidth = 8;
          square(x, y, tileWidth, "red", "#ff8f2f");
          ctx.fillStyle = "black";
          ctx.fillText(str(v) + " - " + str(incx), x, y + 20);
          log(x, y);
          y = this.grid.indexOf(py);
          x = this.grid[y].indexOf(px);
          ...
          for (let i of dir) {
            if (
              ...
              ctx.lineWidth = 2;
              const c = this.grid[i[1]][i[0]];
              square(c.x, c.y, tileWidth, "red", "#a04422");
              ctx.fillStyle = "white";
              ctx.fillText(str(c.v), c.x, c.y + 20);
            }
          }
        }
        incx++;
