...
// the height of the noise, def 0.9
const noiseScale = {
  x: 1,
  y: 1,
};
// how much we de-contrast the noise, def 1500
const df = 1800;
let zoff = 0;
// by what messure we interpret the noise, def  1
const noiseEffect = {
  x: 5,
  y: 10,
};
...
const material = {
  ...
  checkHeight(height, type) {
    ...
    switch (type) {
      ...
      // sand
      case this.sand:
        return height > sandh.max
          ? scaleNum(height, 0, maxTileYHeight, sandh.min, sandh.max)
          : height;
      // grass
      case this.grass:
        return height > grassh.max
          ? scaleNum(height, 0, maxTileYHeight, grassh.min, grassh.max)
          : height;
      // earth
      case this.earth:
  ...
  setup() {
    ...
    for (let x = 0; x < gridSize; x++) {
      ...
      for (let y = 0; y < gridSize; y++) {
        // is a diamond point
        if (y % initColCount === 0 && x % initColCount === 0)
          row.push({
            x: x * tileWidth,
            y: y * tileWidth,
            v: randint(360),
  ...
  calcualteNeighbours(x, y) {
    ...
    for (let i of dir) {
      if (
        i[0] >= 0 &&
        i[1] >= 0 &&
        i[0] <= this.grid[0].length - 1 &&
        i[1] <= this.grid.length - 1
      ) {
  ...
  dev(markInitalPoints = false) {
    for (let py of this.grid) {
      for (let px of py) {
        ...
        if (
          markInitalPoints &&
          this.grid.indexOf(px) % initColCount === 0 &&
          px.indexOf(py) % initColCount === 0
        )
          square(x, y, tileWidth, "blue", "blue");
...
function generateGrid() {
  ...
  return gd.grid.map((x) =>
    x.map((i) => {
      const mat = material.hslConverter(i.v);
      const height = material.checkHeight(
        toFixed(randfloat(maxTileYHeight), terrainHumanize),
        mat
      );
      return [mat, height];
    })
...
const animate = async () => {
  ...
  for (let x of range(grid.length)) {
    for (let y of range(grid[0].length)) {
      ...
      const v = rotateVector(y * noiseScale.y, x * noiseScale.x, angle);
      drawBLock(
        sin(zoff) * x + v.x * noiseEffect.x,
        cos(zoff) * y + v.y * noiseEffect.y,
        c[1],
        c[0]
      );
    }
