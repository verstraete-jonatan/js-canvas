const _APP = async () => {
  ...
  const randint = (a = 1, b = 0, rounded = true) =>
  ...
  class Drop extends All {
    constructor() {
      ...
      this.width = this.speed * toFixed(Math.log(Math.random())) * 10;
      // this.speed = 0.1 + randint(10) / 20;
      // this.x = randint(w);
      // this.y = randint(h);
      // -20;

      // this.width = 5; // this.speed * 10;
      this.color = 0;
    }
    draw() {
      ...
      if (imgData?.length && isInFrame(x, y)) {
        const [r, g, b] = getColorAtPoint(Math.round(x), Math.round(y));
        if (isNaN((r * 3 + g + b) / 100 / 2)) {
          // console.log({ x, y, v: (r * 3 + g + b / 100) / 2 });
          debugger;
        } else if (r + g + b > 0) {
          colorValue = (r * 3 + g + b / 100) / 2000;
          color = `rgb(${r} ${g} ${b})`;
        }
      }

      const [vx, vy] = this.getNoise(x, y, colorValue);

      circle(x + vx, y + vy, width, null, color);
