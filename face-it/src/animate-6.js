const _APP = async () => {
  ...
  const virtualCnv = document.getElementById("virtual");
  const virtualCtx = virtualCnv.getContext("2d", {
  ...
  virtualCnv.height = window.innerHeight;
  virtualCnv.width = window.innerWidth;
  ...
  const toFixed = (nr) => +nr.toFixed(3);
  const degRad = (i) => i * (PI / 180);
  const hsl = (v, s = 50, l = 50, a = 1) => `hsl(${v},${s}%, ${l}%, ${a})`;
  const isInFrame = (x, y) => x < w && y < h && x > 0 && y > 0;
  const randint = (a = 1) => Math.floor(Math.random() * a);
  const distanceTo = (ax, ay, bx, by) => Math.hypot(bx - ax, by - ay);
  ...
  function rotateVector(x, y, angle, rad = 1, isDeg = false) {
    ...
    return [toFixed(_x * cs - _y * sn), toFixed(_x * sn + _y * cs)];
  }
  ...
  class Line {
    constructor(x) {
      ...
      const inc = h / densityY;
      [...Array(densityY)].forEach((_, i) => {
        this.points.push(new Point(x, inc * i));
      });
    }
    draw() {
      ...
      this.points.forEach((i) => {
        pr = i.move(pr);
        ctx.lineTo(i.drawX, i.drawY);
        // ctx.fillText(pr.toFixed(), i.drawX, i.drawY);
      });
      ctx.stroke();
    }
  }
  ...
  class Point {
    ...
    getNoise(x, y, colorValue) {
      const { df, zoff, scale } = noiseSetup;
      const value = noise.perlin3(x / df, y / df, zoff);

      const angle = ((1 + value) * 1.1 * 128) / (PI * 2);
      return rotateVector(x * scale, y, angle);
    }
    move(prevR = 0) {
      const { x, y } = this;
      ...
      const [vx, vy] = this.getNoise(x, y);
      this.drawX += vx;
      this.drawY += vy;

      if (imgData?.length) {
        let [r, g, b] = getColorAtPoint(Math.round(x), Math.round(y));

        // r = r + g * 0.5 + b * 0.1;

        if (!!(r + g + b)) {
          const neg = x > faceCenter[0] ? 1 : -1;
          const v = r * 5 + g + b * 5 * neg;
          this.drawX += v / 15;
          // this.drawY += r / 5;
        } else {
          this.drawX += 80;
    ...
    //   let color = hsl(this.color + gloablColor, 40, 20, 0.6);

    //   this.drawX = x;
    //   this.drawY = y;

    //   const [vx, vy] = [1, 1]; // this.getNoise(x, y, colorValue);
    //   this.drawX += vx;
    //   this.drawY += vy;
  ...
  const drawFacePoints = (_ctx, points) => {
    _ctx.strokeStyle = _ctx.fillStyle = hsl(
      50 + points[0][2] + points[1][2] + points[2][2]
    );
    _ctx.beginPath();
    _ctx.moveTo(points[0][0], points[0][1]);
    for (const pt2 of points) {
      _ctx.lineTo(Math.trunc(pt2[0]), Math.trunc(pt2[1]));
    }
    _ctx.closePath();
    _ctx.stroke();
    _ctx.fill();
  ...
  const drawFaceGrid = () => {
    ...
    if (mesh) {
      const [x, y, x2, y2] = window.humanInstance?.result.face[0].box;
      faceCenter = [x2 - x, y2 - y];
      for (let i = 0; i < TRI468.length / 3; i++) {
        drawFacePoints(virtualCtx, [
          mesh[TRI468[i * 3 + 0]],
          mesh[TRI468[i * 3 + 1]],
          mesh[TRI468[i * 3 + 2]],
        ]);
  ...
  document.addEventListener("click", function ({ clientX: x, clientY: y }) {
    const [r, g, b] = virtualCtx.getImageData(x, y, 1, 1).data;
  ...
  async function main() {
    const inc = w / densityX;
    const lines = [...Array(densityX)].map((_, x) => new Line(x * inc));
    ...
    async function animation() {
      ...
      drawFaceGrid();
      imgData = virtualCtx.getImageData(0, 0, w, h).data;

      for (let i of lines) i.draw();
...
plan 5:
To really make this work, we need to create a field of the triangles of the face. With this field, we can then calculate the steepness of a point on a triangle.

POC:
- create triangle field
- create a 2D vector that follows the cursor
- calculate the steepness (if applicable) of the position of the vector on the triangle
- display the steepness in color as you move the cursor


Plan 6: 
To simplify everything, use another canvas to draw the face in colors by depth. Then simply extract those pixels to use as depth value.
