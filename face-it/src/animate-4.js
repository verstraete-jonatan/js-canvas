const _APP = async () => {
  ...
  class All {
    ...
    getNoise(x, y, colorValue) {
      ...
      const value = colorValue * 1; // noise.perlin3(x / df, y / df, zoff) * scale;
  ...
  class Drop extends All {
    ...
    draw() {
      ...
      let colorValue = 1;
      let color = this.color + gloablColor;
      color = 0;

      if (imgData?.length && x * y > 0) {
      ...
      circle(x + v.x, y + v.y, width, hsl(color));
  ...
  const drawFaceGrid = () => {
    ...
    // let min = Math.min(...mesh.flat());
    // min = min < 0 ? Math.abs(min) : 0;
    // console.log(Math.min(...mesh.flat()), Math.max(...mesh.flat()));
    ...
    for (let i = 0; i < TRI468.length / 3; i++) {
      ...
      // drawFacePoints(ctx, points);
      drawFacePoints(virtualCtx, points);
    }
  };
