...
class Ant {
  constructor(colony) {
    ...
    this.colony = colony;
    this.x = colony.x;
    (this.y = colony.y), (this.color = COLORS.next()); //colony.flag
  ...
  getLineOfSight() {
    ...
    for (let _x of range(this.lineOfSight * 2).map((i) =>
      floor(x + i - this.lineOfSight)
    )) {
      for (let _y of range(this.lineOfSight * 2).map((i) =>
        floor(y + i - this.lineOfSight)
      )) {
        // is grid point
  ...
  speadPheromone() {
    ...
    if (this.spreadCount > this.scentInterval) {
      ...
      if (!this.outBounds && this.target === this.colony) {
        ...
        a.pheromone += 1;
        a.pheromoneOrientation =
          (this.orientation - degRad(360)) * this.accuracy;
      }
    }
  }
}
