...
class Point {
  constructor(id) {
    this.id = id;
    this.alpha = 1;
    this.theta = tan(id / 10000); //PI + cos(id / 100) * 100; //+cos(id/20)//randfloat(0, PI2, 5)
    this.phi = id * 1; //PI + tan(id * 100) * 100; //+tan(id*100)//randfloat(0, PI, 5)
    this.projScale = 0;
    this.projX = 0;
    this.projY = 0;
    this.x = 0;
  ...
  move() {
    ...
    //this.phi += 0.01 * setup.rotHorizontal
...
class Sphere {
  constructor() {
    ...
    // this.points = this.points.sortDc("phi");

    for (let i of this.points) {
      i.linked = this.points.next();
    }
    this.colorScale = 360;
    ctx.lineWidth = 0.5;
  }
  animate() {
    ...
    //line(i.projX, i.projY, i.linked.projX, i.linked.projY, 'red')
    // line(i.projX, i.projY, i.projX + x, i.projY + y, "red");
    //dot(i.projX, i.projY,5)
    //}
    ...
    for (let i of this.points) {
      ...
      ctx.lineTo(i.linked.projX, i.linked.projY);
